...
int main(int argc, char *argv[])
{
    ...
    // reduce(Foam::label, Foam::sumOp<double>)
    // test code, the output is 4.0
    // scalar testvalue(1.);
    // reduce(testvalue, sumOp<scalar>());
    // Info << "Total testvalues on all processors: " << testvalue << endl;


    // the reduce will add the values from all processors together
    // reduce is a function
    // returnReduce will give the final value
    ...
    // List<label> test (Pstream::nProcs());
    // Info<< test[0] << " , " << test[1] << endl << endl;
    ...
    // Info<< endl << "nInternalFaces: " << nInternalFaces << endl << endl;
    // Info<< endl << "nBoundaries: " << nBoundaries << endl << endl;
