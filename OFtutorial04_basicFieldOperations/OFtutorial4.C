...
int main(int argc, char *argv[])
{
	...
	Info<< "Create field Ux\n" << endl;
	volScalarField Ux // note that velocity is a vector field
	(
		IOobject
		(
		    "Ux",
	...
	const vector originVector2(0.0,0.0,0.00);
	const vector originVector3(1.0,1.0,0.00);
	...
	const scalar rFarCell = max( // find the maximum value from all distances
		...
		// RSH mesh.C() is the vector of the center of all cell
		mag(dimensionedVector("x0",dimLength,originVector) - mesh.C())
		).value(); // convert to dim-less scalar
	
	Info<< "\nrFarCell = " << rFarCell << endl << endl;
	Info<< mag(dimensionedVector("00",dimLength,originVector3) - 
	       dimensionedVector("00",dimLength,originVector2)).value() << endl;
	Info<< endl << dimensionedScalar("tmp",dimTime,1.) << endl;
	// Info<< endl << 
...
scalar calculatePressure(scalar t, vector x, vector x0, scalar scale)
{
	...
	return Foam::sin(Foam::constant::mathematical::pi*f*t)*rR;
}
