...
:
    ...
    // Rsh, following parameters are readed by the read function, line 204 to 224
    ...
    V_(0.0)
{
    ...
    // Rsh, mesh.C() is the center coordinate of the cells
    // Rsh, x0 is the center vector of the disk 
    scalarField R (mag(mesh.C() - x0)); // distance
    vectorField rHat ((mesh.C() - x0) / mag(mesh.C() - x0)); //unit distance vector
    ...
    for (label cellI = 0; cellI < mesh.C().size(); cellI++)
    {
        ...
        scalar dNormal = R[cellI] * (rHat[cellI] & diskDir_); // distance vector & unit normal direction vector
